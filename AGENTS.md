...
| File | Purpose |
|---|---|
| `__init__.py` | Exports: `OptimizerConfig`, `NetworkOptimizer`, `SearchResult`, `access_score`, `compute_coverage`, `weighted_objective`, `CandidateRanker`, `load_*` |
...
| `scoring.py` | BallTree coverage queries + access scoring (`access_score`, `compute_coverage`, `weighted_objective`, `normalize_to_100`, `miles_to_radians`) |
...
- **Weights** (JSON): `{column_name: weight}` — e.g. `{"efficiency": 0.3, "effectiveness": 0.2}`. Values in [0, 1], sum < 1.0. access gets remaining weight.
...
## Performance Notes
- `access_score` on full pool (124K providers, 10K members, 100 thresholds): ~0.79s
